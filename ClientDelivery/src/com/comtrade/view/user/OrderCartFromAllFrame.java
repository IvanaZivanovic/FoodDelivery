...
public class OrderCartFromAllFrame extends JFrame {
	 ...
	 */
	public OrderCartFromAllFrame(User user, String restName) throws ClassNotFoundException, IOException {
		Restaurant restaurant = new Restaurant();
		restaurant.setName(restName);
		
		TransferObject requestRest = TransferObject.create(restaurant, ConstantFc.RESTAURANT, ConstantBl.GET_ONE_R_NAME);
		TransferObject responseRest = FrontController.getInstance().execute(requestRest);
		r = (Restaurant) responseRest.getResponse();
		u = user;
		...
		contentPane = new JPanel(){  
            public void paintComponent(Graphics g) {  
                Image img = Toolkit.getDefaultToolkit().getImage(  
                		OrderCartFromAllFrame.class.getResource("/images/order-wallpaper.JPG"));  
		...
		lblMinimalOrder.setBounds(857, 50, 128, 14);
		contentPane.add(lblMinimalOrder);
		
		lblMinRest = new JLabel(String.valueOf(r.getMinOrder()));
		lblMinRest.setFont(new Font("Tahoma", Font.PLAIN, 14));
		lblMinRest.setForeground(Color.WHITE);
		lblMinRest.setBounds(891, 93, 94, 14);
		...
		for (Address a : addresses) {
			...
			btnAddAddress.setBounds(730, 34, 117, 23);
